...
// ============ debounce utility ============
    ...
    // ============ 📊 运行时变量（P1 自 dsh-tavern-v2「🔢 变量面板」整体迁入）============
    //
    // 数据源不变，全是 muv-engine 的同源 HTTP（本仓库不改服务端）：
    //   GET  /api/muv-engine/state?sessionId=…             → { ok, state:{ data:<树> } }
    //   POST /api/muv-engine/state { sessionId, merge:true, data:{…} }  深合并写回（只 merge 不删除）
    //   GET  /api/muv-engine/state?sessionId=…&snapshots=1  → { ok, snapshots:[{ key, at, data }] }
    //   GET  /api/muv-engine/state?sessionId=…&messageKey=… → 按楼回看（入口探测失败 ⇒ 隐藏）
    //
    // 防双滚动：新区嵌在 MUV 面板既有的滚动容器里（内容区 flex:1 + 自动滚动那层），
    // 自己不自带任何滚动样式 —— 旧面板 max-height:340px 内滚的双滚动条问题就此消灭。
    //
    // ★ 下面这段纯函数被 test-var-section.mjs **逐字提取执行**：禁止引用 React / DOM /
    //   外层闭包变量，只用入参 + 全局 fetch；两端标记行保持原样。
    // [[var-section-core-start]]
    const VAR_PATH_SEP = '\u0001'

    function varSectionTypeOf(v) {
      if (v === null) return 'null'
      if (Array.isArray(v)) return 'array'
      return typeof v
    }

    function varSectionPathLabel(arr) { return arr.map(function (k) { return String(k) }).join('.') }

    // 按路径数组在树里找原始值（找不到 → undefined）
    function varSectionValueAt(root, arr) {
      var cur = root
    ...
    // 「已覆盖」三态：initvarData 无此键 → false；值相同 → false；值（类型或值）不同 → true
    function varSectionIsOverridden(initialTree, arr, value) {
      if (!initialTree || typeof initialTree !== 'object') return false
      var iv = varSectionValueAt(initialTree, arr)
      if (iv === undefined) return false
      return JSON.stringify(iv) !== JSON.stringify(value)
    }

    // 叶子展示信息：null / 数组不可编辑，只显示文本（文本是原样字符串，由上层以
    // children=textContent 方式渲染，任何标签都不会变成真实元素）
    function varSectionLeafDisplay(value) {
      var t = varSectionTypeOf(value)
      if (t === 'null') return { editable: false, text: 'null' }
      if (t === 'array') return { editable: false, text: '[数组 ' + value.length + ' 项]' }
      return { editable: true, type: t, text: String(value) }
    }

    // 编辑还原原始类型（找不到原叶型按字符串存；number 转不回去就保留字符串）
    function varSectionNormalizeEdit(origType, raw) {
      if (origType === 'boolean') return !!raw
      if (origType === 'number') { var n = Number(raw); return isNaN(n) ? String(raw) : n }
      return String(raw)
    }

    // edits 表 → 嵌套 payload（merge 写回用）。值**原样**提交：null / 0 / false / ''
    // 都不做任何 coerce —— merge 语义会不会吃掉它由提交形状保证。
    function varSectionEditsToPayload(edits) {
      var payload = {}
      for (var pk in (edits || {})) {
        var arr = pk.split(VAR_PATH_SEP)
        var cur = payload
    ...
    // 保存结果 → 新 edits 表：失败/无会话**保留**（本地修改可重试），成功才清空
    function varSectionApplySaveResult(prevEdits, result) {
      if (result && result.kind === 'ok') return {}
      return prevEdits
    }

    // 首次读取默认展开：只展开**顶层**的对象分支（懒展开 —— 深层点开才渲染）
    function varSectionDefaultExpanded(tree) {
      var expanded = {}
      if (!tree || typeof tree !== 'object') return expanded
      for (var tk in tree) {
        var tv = tree[tk]
    ...
    // 搜索：命中路径/值的叶子拍平（限 200 条防炸）；无命中 → []
    function varSectionSearchHits(tree, query, limit) {
      ...
      function walk(node, arr) {
        ...
        for (var k in node) {
          ...
          if (v !== null && typeof v === 'object' && !Array.isArray(v)) walk(v, arr2)
            ...
            var hay = (varSectionPathLabel(arr2) + ' ' + String(v)).toLowerCase()
            if (hay.indexOf(q) >= 0) hits.push({ path: arr2, value: v })
            if (hits.length >= cap) return
          }
        }
      }
    ...
    async function varSectionLoadState(input) {
      var inp = input || {}
      var callFetch = inp.fetch || (typeof fetch === 'function' ? fetch : null)
      var sessionId = String(inp.sessionId || '')
      if (!sessionId) return { kind: 'no-session', message: '未检测到当前会话 —— 打开一个对话后再看变量', tree: null }
      if (!callFetch) return { kind: 'error', message: '当前环境不支持 fetch', tree: null }
      try {
        var r = await callFetch('/api/muv-engine/state?sessionId=' + encodeURIComponent(sessionId))
        var d = await r.json()
        if (!d || !d.ok) throw new Error((d && d.error) || '读取失败')
        var tree = (d.state && d.state.data) || null
        return { kind: 'ok', tree: (tree && typeof tree === 'object') ? tree : null }
      } catch (e) {
        return { kind: 'error', message: '读取变量失败：' + ((e && e.message) || String(e)), tree: null }
      }
    }

    async function varSectionSave(input) {
      var inp = input || {}
      var callFetch = inp.fetch || (typeof fetch === 'function' ? fetch : null)
      var sessionId = String(inp.sessionId || '')
      var edits = inp.edits || {}
      var n = Object.keys(edits).length
      if (!sessionId) return { kind: 'no-session', message: '未检测到当前会话，写不回去', keepEdits: true }
      if (!n) return { kind: 'nothing', message: '没有未保存的修改', keepEdits: true }
      if (!callFetch) return { kind: 'error', message: '当前环境不支持 fetch（本地修改已保留，可重试）', keepEdits: true }
      try {
        var r = await callFetch('/api/muv-engine/state', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ sessionId: sessionId, merge: true, data: varSectionEditsToPayload(edits) })
        })
        var d = await r.json()
        if (!d || !d.ok) throw new Error((d && d.error) || '写回失败')
        return { kind: 'ok', count: n, message: '已写回 ' + n + ' 处（merge）—— 卡下一轮查询/推送就能拿到新值' }
      } catch (e) {
        return { kind: 'error', message: ((e && e.message) || String(e)) + '（本地修改已保留，可重试）', keepEdits: true }
      }
    }

    // ⏱ 快照列表探测：任何失败（404 / 网络错 / 快照为空）→ kind:'hidden'，入口整行隐藏
    async function varSectionListSnapshots(input) {
      var inp = input || {}
      var callFetch = inp.fetch || (typeof fetch === 'function' ? fetch : null)
      var sessionId = String(inp.sessionId || '')
      if (!sessionId || !callFetch) return { kind: 'hidden', message: '快照不可用' }
      try {
        var r = await callFetch('/api/muv-engine/state?sessionId=' + encodeURIComponent(sessionId) + '&snapshots=1')
        var d = await r.json()
        var list = (d && d.ok && Array.isArray(d.snapshots)) ? d.snapshots : null
        if (!list || !list.length) return { kind: 'hidden', message: '无快照' }
        return { kind: 'ok', snapshots: list }
      } catch (_) {
        return { kind: 'hidden', message: '快照接口不可用' }   // 探测失败 → 静默隐藏，不报错不假装有数据
      }
    }

    // ⏱ 进入某楼回看：messageKey 拉那层的树（响应形状与普通 GET 一致）；失败如实报错。
    //   回看视图由调用方进入只读模式 —— 快照绝不 merge 写回。
    async function varSectionEnterSnapshot(input) {
      var inp = input || {}
      var callFetch = inp.fetch || (typeof fetch === 'function' ? fetch : null)
      var sessionId = String(inp.sessionId || '')
      var messageKey = String(inp.messageKey || '')
      if (!sessionId || !messageKey || !callFetch) return { kind: 'error', message: '该楼快照不存在' }
      try {
        var r = await callFetch('/api/muv-engine/state?sessionId=' + encodeURIComponent(sessionId) + '&messageKey=' + encodeURIComponent(messageKey))
        var d = await r.json()
        if (!d || !d.ok || !d.state) throw new Error('该楼快照不存在')
        return { kind: 'ok', tree: (d.state && d.state.data) || null }
      } catch (e) {
        return { kind: 'error', message: '回看失败：' + ((e && e.message) || String(e)) }
      }
    }

    // 快照下拉项的展示标签（时间 + 键尾 8 位；过大未存的禁选提示）
    function varSectionSnapLabel(s) {
      s = s || {}
      ...
      if (n > 0) {
        ...
        when = d.getFullYear() + '-' + p2(d.getMonth() + 1) + '-' + p2(d.getDate()) + ' ' + p2(d.getHours()) + ':' + p2(d.getMinutes())
      }
      var tail = key.length > 8 ? key.slice(-8) : key
      return when + ' · …' + tail + (s.data == null ? '（过大未存）' : '')
    }

    // 导出文本：没有树 → null（调用方提示"还没有变量可导出"）
    function varSectionExportText(tree) {
      if (!tree || typeof tree !== 'object') return null
      return JSON.stringify(tree, null, 2)
    }
    // [[var-section-core-end]]

    // ── 📊 运行时变量（React 只是壳，逻辑在上面纯函数里）──────────────────
    // XSS 纪律：变量值是外部数据，一律走 children（= textContent）渲染，
    // 本区不做任何 HTML 字符串拼接。
    function RuntimeVarsSection({ sessionId, initvarData, reloadToken }) {
      const [open, setOpen] = React.useState(false)           // 默认折叠，点标题展开/收起
      const [tree, setTree] = React.useState(null)
      const [edits, setEdits] = React.useState({})            // pathKey -> { value, type }（未保存的本地修改）
      const [expanded, setExpanded] = React.useState({})      // pathKey -> true（懒展开：点开才渲染子层）
      const [query, setQuery] = React.useState('')
      const [snapList, setSnapList] = React.useState(null)    // null ⇒ 快照入口整行隐藏
      const [snapMode, setSnapMode] = React.useState(false)   // ⏱ 回看模式：整树只读，绝不写回
      const [status, setStatus] = React.useState(null)        // { text, tone }

      const boundFetch = () => (typeof fetch === 'function' ? fetch.bind(window) : null)
      const TC = {
        ok: 'var(--dsw-alias-state-success-primary,#27ae60)',
        busy: 'var(--dsw-alias-state-business-primary,#3498db)',
        warn: 'var(--dsw-alias-state-warn-primary,#e67e22)',
        error: 'var(--dsw-alias-state-error-primary,#e74c3c)',
        dim: C.dim
      }
      const btnSm = { padding: '2px 8px', borderRadius: 6, border: '1px solid ' + C.border, background: C.bg3, color: C.text, cursor: 'pointer', fontSize: 11, fontFamily: 'inherit' }

      const doLoad = async (quiet) => {
        if (!quiet) setStatus({ text: '⏳ 读取变量中…', tone: 'busy' })
        const r = await varSectionLoadState({ sessionId, fetch: boundFetch() })
        if (r.kind === 'ok') {
          setTree(r.tree)
          setExpanded(varSectionDefaultExpanded(r.tree))
          const top = r.tree ? Object.keys(r.tree).length : 0
          const pending = Object.keys(edits).length
          setStatus({ text: '✅ 已读取' + (top ? '（顶层 ' + top + ' 个分支）' : '（空树）') + (pending ? ' · 有 ' + pending + ' 处未保存修改' : ''), tone: 'ok' })
          // ⏱ 静默探测快照入口：探测失败 ⇒ snapList 保持 null，整行隐藏
          const sr = await varSectionListSnapshots({ sessionId, fetch: boundFetch() })
          setSnapList(sr.kind === 'ok' ? sr.snapshots : null)
        } else {
          setTree(null)
          setStatus({ text: (r.kind === 'no-session' ? '⚠️ ' : '❌ ') + r.message, tone: r.kind === 'no-session' ? 'warn' : 'error' })
        }
      }

      // 会话/预设切换不另挂监听：MuvTableView 的 check() 消费同一批 forceReload
      // 监听后 bump reloadToken，这里跟着重读；sessionId 变了 ⇒ 旧会话的本地
      // edits / 回看状态一并丢弃（避免把 A 会话的修改 merge 进 B 会话）。
      const prevSid = React.useRef(sessionId)
      React.useEffect(() => {
        if (!open) return
        if (prevSid.current !== sessionId) { prevSid.current = sessionId; setEdits({}); setSnapMode(false); setQuery('') }
        doLoad(false)
      }, [open, sessionId, reloadToken])

      // 🔄 刷新：丢弃本地修改 + 退出回看，重新从服务端读
      const doRefresh = () => { setEdits({}); setSnapMode(false); setQuery(''); doLoad(false) }

      const doSave = async () => {
        if (snapMode) { setStatus({ text: '⛔ 正在回看历史楼层的变量（只读）—— 点「回到最新」退出后再修改', tone: 'warn' }); return }
        const n = Object.keys(edits).length
        setStatus({ text: n ? '⏳ 写回 ' + n + ' 处修改…' : '⏳ …', tone: 'busy' })
        const r = await varSectionSave({ sessionId, edits, fetch: boundFetch() })
        setEdits(prev => varSectionApplySaveResult(prev, r))
        if (r.kind === 'ok') {
          setStatus({ text: '✅ ' + r.message, tone: 'ok' })
          doLoad(true)
        } else {
          setStatus({ text: (r.kind === 'error' ? '❌ ' : r.kind === 'no-session' ? '⚠️ ' : '') + r.message, tone: r.kind === 'error' ? 'error' : 'warn' })
        }
      }

      const doExport = async () => {
        const text = varSectionExportText(tree)
        if (!text) { setStatus({ text: '还没有变量可导出', tone: 'warn' }); return }
        try {
          if (navigator.clipboard && navigator.clipboard.writeText) {
            await navigator.clipboard.writeText(text)
            setStatus({ text: '✅ 已复制到剪贴板（' + text.length + ' 字符）', tone: 'ok' })
            return
          }
          ...
          try {
            ...
            setStatus({ text: '✅ 已复制到剪贴板（' + text.length + ' 字符）', tone: 'ok' })
          } catch (_e) { setStatus({ text: '❌ 复制失败，浏览器拒绝了剪贴板访问', tone: 'error' }) }
        }
      }

      const enterSnap = async (key) => {
        if (!key) { exitSnap(); return }
        setStatus({ text: '⏳ 正在拉取该楼快照…', tone: 'busy' })
        const r = await varSectionEnterSnapshot({ sessionId, messageKey: key, fetch: boundFetch() })
        if (r.kind === 'ok') {
          setSnapMode(true); setEdits({}); setQuery('')
          setTree(r.tree)
          setExpanded(varSectionDefaultExpanded(r.tree))
          setStatus({ text: '⏱ 正在回看历史楼层的变量，点「回到最新」恢复', tone: 'busy' })
        } else {
          setStatus({ text: '❌ ' + r.message, tone: 'error' })
        }
      }
      const exitSnap = () => { setSnapMode(false); doLoad(false) }

      const setEdit = (pk, origType, raw) => setEdits(prev => ({
        ...prev,
        [pk]: { value: varSectionNormalizeEdit(origType, raw), type: ['boolean', 'number', 'string'].indexOf(origType) >= 0 ? origType : 'string' }
      }))

      const inputStyle = { flex: 1, minWidth: 120, padding: '2px 6px', fontSize: 12, background: C.bg3, color: C.text, border: '1px solid ' + C.border, borderRadius: 4, fontFamily: 'inherit', outline: 'none' }

      const renderLeaf = (arr, value) => {
        const pk = arr.join(VAR_PATH_SEP)
        const edited = edits[pk]
        // ★ 已编辑的叶子回显**编辑后的值**（否则重渲染时输入框被重置回旧值）；
        //   「已覆盖」对照的是**服务端值**，回看模式下不标（对照当前卡 initvar 对历史楼层语义错位）
        const displayValue = edited !== undefined ? edited.value : value
        const displayType = varSectionTypeOf(displayValue)
        const overridden = !snapMode && varSectionIsOverridden(initvarData, arr, value)
        const info = varSectionLeafDisplay(displayValue)
        const indent = Math.min(arr.length - 1, 6) * 14 + 4
        const editor = !info.editable
          ? jsx.jsx('span', { style: { fontSize: 11, color: C.dim }, children: info.text })
          : displayType === 'boolean'
            ? jsx.jsxs('label', { style: { cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }, children: [
                jsx.jsx('input', { type: 'checkbox', checked: !!displayValue, onChange: e => setEdit(pk, 'boolean', e.target.checked) }),
                displayValue ? 'true' : 'false'
              ] })
            : jsx.jsx('input', { type: 'text', value: String(displayValue), onChange: e => setEdit(pk, varSectionTypeOf(value), e.target.value), style: inputStyle })
        return jsx.jsxs('div', {
          'data-dsh-muv-var-leaf': pk,
          style: { display: 'flex', alignItems: 'center', gap: 6, padding: '1px 0 1px ' + indent + 'px' },
          children: [
            jsx.jsx('span', { style: { color: C.accent, fontSize: 12 }, children: String(arr[arr.length - 1]) }),
            overridden && jsx.jsx('span', { title: '与卡声明的初始值不同', style: { fontSize: 10, color: TC.warn }, children: '已覆盖' }),
            edited !== undefined && jsx.jsx('span', { style: { fontSize: 10, color: TC.ok }, children: '●未保存' }),
            jsx.jsx('span', { style: { flex: 1 } }),
            jsx.jsx('span', { style: { flex: 2, display: 'flex', alignItems: 'center', gap: 6 }, children: editor })
          ]
        }, pk)
      }

      // 渲染一棵子树：只渲染到已展开层级（懒展开，不一次性渲染上千节点）
      const renderNode = (node, arr) => {
        const out = []
        const keys = Object.keys(node || {}).sort((a, b) => {
          const ao = node[a] !== null && typeof node[a] === 'object' && !Array.isArray(node[a])
          const bo = node[b] !== null && typeof node[b] === 'object' && !Array.isArray(node[b])
          if (ao !== bo) return ao ? 1 : -1   // 叶子在前，对象在后（与旧面板一致）
          return a < b ? -1 : a > b ? 1 : 0
        })
        for (let i = 0; i < keys.length; i++) {
          const k = keys[i]
          const v = node[k]
          const arr2 = arr.concat([k])
          const t = varSectionTypeOf(v)
          const pk = arr2.join(VAR_PATH_SEP)
          if (t === 'object' && Object.keys(v).length > 0) {
            const isOpen = !!expanded[pk]
            out.push(jsx.jsxs('div', {
              'data-dsh-muv-var-node': pk,
              onClick: () => setExpanded(prev => ({ ...prev, [pk]: !prev[pk] })),
              style: { cursor: 'pointer', userSelect: 'none', padding: '1px 0 1px ' + (Math.min(arr.length - 1, 6) * 14) + 'px' },
              children: [
                jsx.jsx('span', { style: { color: C.dim, fontSize: 10 }, children: isOpen ? '▼' : '▶' }),
                jsx.jsx('span', { style: { color: 'var(--dsw-alias-state-business-primary,#3498db)', fontSize: 12 }, children: String(k) }),
                jsx.jsx('span', { style: { fontSize: 10, color: C.dim }, children: ' {' + Object.keys(v).length + '}' })
              ]
            }, pk))
            if (isOpen) out.push(jsx.jsx('div', { children: renderNode(v, arr2) }, pk + '::sub'))
          } else if (t === 'object' || (t === 'array' && v.length === 0)) {
            out.push(renderLeaf(arr2, t === 'object' ? {} : v))
          } else {
            out.push(renderLeaf(arr2, v))
          }
        }
        return out
      }
      ...
      const searchHits = (q && tree && typeof tree === 'object') ? varSectionSearchHits(tree, q) : null
      const treeBody = !tree || typeof tree !== 'object'
        ? jsx.jsx('div', { style: { color: C.dim, padding: 8 }, children: '（当前会话还没有变量 —— 先和卡聊一轮，或导入带 initvar 的卡）' })
        : searchHits
          ? (searchHits.length === 0
              ? jsx.jsx('div', { style: { color: C.dim, padding: 8 }, children: '没有匹配的变量' })
              : jsx.jsxs('div', { children: [
                  jsx.jsx('div', { style: { fontSize: 10, color: C.dim, marginBottom: 4 }, children: '匹配 ' + searchHits.length + ' 条叶子' + (searchHits.length >= 200 ? '（已截断）' : '') + '：' }),
                  searchHits.map(h => jsx.jsxs('div', { key: 'hit::' + varSectionPathLabel(h.path), children: [
                    jsx.jsx('div', { style: { fontSize: 10, color: C.dim, wordBreak: 'break-all' }, children: varSectionPathLabel(h.path) }),
                    renderLeaf(h.path, h.value)
                  ] }))
                ] }))
          : renderNode(tree, [])

      return jsx.jsxs('div', {
        'data-dsh-muv-var-section': '',
        style: { flexShrink: 0, borderBottom: '1px solid ' + C.border, background: C.bg2 },
        children: [
          // 标题行（点击展开/收起，默认折叠）
          jsx.jsxs('div', {
            'data-dsh-muv-var-toggle': '',
            onClick: () => setOpen(o => !o),
            style: { display: 'flex', alignItems: 'center', gap: 8, padding: '7px 14px', cursor: 'pointer', userSelect: 'none' },
            onMouseEnter: e => { e.currentTarget.style.background = C.hover },
            onMouseLeave: e => { e.currentTarget.style.background = 'transparent' },
            children: [
              jsx.jsx('span', { style: { fontSize: 10, color: C.dim, width: 12 }, children: open ? '▾' : '▸' }),
              jsx.jsx('span', { style: { fontWeight: 600, fontSize: 12, flex: 1 }, children: '📊 运行时变量' }),
              jsx.jsx('span', { style: { fontSize: 10, color: C.dim }, children: '查看 · 编辑写回（merge）· 按楼回看' })
            ]
          }),
          open && jsx.jsxs('div', { style: { padding: '0 14px 12px', display: 'flex', flexDirection: 'column', gap: 8 }, children: [
            // 工具行：搜索 / 刷新（丢弃本地改动）/ 保存（merge 写回）/ 导出 JSON
            jsx.jsxs('div', { style: { display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }, children: [
              jsx.jsx('input', {
                'data-dsh-muv-var-search': '', type: 'text', placeholder: '🔍 搜索变量路径或值…',
                value: query, onChange: e => setQuery(e.target.value),
                style: { flex: 1, minWidth: 140, padding: '3px 8px', fontSize: 12, background: C.bg3, color: C.text, border: '1px solid ' + C.border, borderRadius: 4, fontFamily: 'inherit', outline: 'none' }
              }),
              jsx.jsx('button', { 'data-dsh-muv-var-refresh': '', type: 'button', onClick: doRefresh, title: '重新从服务端读取（丢弃未保存的本地修改）', style: btnSm, children: '🔄 刷新' }),
              jsx.jsx('button', { 'data-dsh-muv-var-save': '', type: 'button', onClick: doSave, title: '把已编辑的叶子写回服务端（merge）', style: btnSm, children: '💾 保存修改' + (Object.keys(edits).length ? '（' + Object.keys(edits).length + '）' : '') }),
              jsx.jsx('button', { 'data-dsh-muv-var-export': '', type: 'button', onClick: doExport, title: '把当前变量树复制为 JSON', style: btnSm, children: '📋 导出 JSON' })
            ] }),
            // ⏱ 按楼回看：探测失败 ⇒ snapList 为 null，整行不渲染（不报错不占位）
            snapList && snapList.length > 0 && jsx.jsxs('div', { 'data-dsh-muv-var-snap-row': '', style: { display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }, children: [
              jsx.jsx('span', { style: { fontSize: 12, color: C.gold }, children: '⏱ 按楼回看' }),
              jsx.jsx('select', {
                'data-dsh-muv-var-snap-select': '',
                value: '',
                onChange: e => enterSnap(e.target.value),
                style: { flex: 1, minWidth: 160, padding: '2px 6px', fontSize: 12, background: C.bg3, color: C.text, border: '1px solid ' + C.border, borderRadius: 4 },
                children: [jsx.jsx('option', { value: '', children: '— 选择楼层回看 —' }, '_none')].concat(
                  snapList.filter(s => s && s.key).map(s => jsx.jsx('option', { value: s.key, disabled: s.data == null, children: varSectionSnapLabel(s) }, s.key))
                )
              }),
              snapMode && jsx.jsx('button', { 'data-dsh-muv-var-back-latest': '', type: 'button', onClick: exitSnap, title: '退出回看，恢复显示最新变量树', style: btnSm, children: '回到最新' })
            ] }),
            jsx.jsx('div', {
              'data-dsh-muv-var-status': '',
              style: { fontSize: 11, lineHeight: 1.6, color: status ? (TC[status.tone] || C.dim) : C.dim },
              children: status ? status.text : '（展开时自动读取当前会话的运行时变量）'
            }),
            // 树：嵌在外层滚动容器里，这里不自带任何滚动样式（防双滚动）
            jsx.jsx('div', {
              'data-dsh-muv-var-tree': '',
              style: { fontSize: 12, lineHeight: 1.9, fontFamily: 'ui-monospace,Consolas,monospace' },
              children: treeBody
            })
          ] })
        ]
    ...
    function MuvTableView({ onClose }) {
      ...
      const [varTick, setVarTick] = React.useState(0)   // 📊 运行时变量区的重读信号（见 check() 内）
          ...
          try {
            ...
            if (d.ok && d.name) {
              ...
              if (syncKey !== lastSyncKey.current || forceReload) {
                forceReload = false
                // 📊 运行时变量区挂在**同一批**监听上：tavern-preset-changed /
                //   tavern-session-changed → forceReload → 这里消费并 bump varTick，
                //   新区（RuntimeVarsSection）凭 reloadToken 变化重读，不另挂第二套监听。
                setVarTick(t => t + 1)
      ...
      return jsx.jsxs('div', { style:{position:'fixed',inset:0,zIndex:60,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center'}, onClick:onClose, children: [
        ...
        // Tree
        // Content（★ 全面板**唯一**的滚动容器：flex:1 + 自动滚动。📊 运行时变量区和
        //   变量树都嵌在这层里滚，新区自己不自带任何滚动样式 —— 防双滚动，
        //   旧面板 max-height 内滚的双滚动条问题就此消灭）
        state.status==='loaded' && jsx.jsxs('div', { style:{flex:1,overflow:'auto',padding:'8px 12px'}, children: [
          // 📊 运行时变量（自 tavern-v2 面板迁入）：sessionId 复用 currentSessionId()，
          //   initvarData 直接用 state 里 tavern-card 加载时带回来的那份，不二次请求。
          jsx.jsx(RuntimeVarsSection, {
            sessionId: currentSessionId(),
            initvarData: state.initvarData,
            reloadToken: varTick
          }),
          state.schemas.length===0 && jsx.jsxs('div', { style:{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:12,padding:'24px 8px',color:C.dim}, children: [
          ...
          state.schemas.length>0 && state.schemas.filter(f=>!state.search||f.name.includes(state.search)||JSON.stringify(f).includes(state.search)).map(field =>
