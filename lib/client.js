...
// ============ debounce utility ============
    ...
    function MuvTableView({ onClose }) {
      ...
      // 会话 id 则不会撒谎：服务端按它查 session-bindings.json，直接得到该会话绑的预设。
      ...
      // 本次请求要带的定位参数。会话 id 优先——它随会话切换而变，是唯一不会「粘住」的依据。
      let lastSessionId = ''
      const resolveActivePreset = async () => {
        lastSessionId = currentSessionId()
        // 有会话 id 就不必猜：服务端会按 session-bindings.json 查出该会话绑的预设。
        if (lastSessionId) return lastPresetId
        // 拿不到会话 id 时才退回酒馆 DOM；DOM 元素在酒馆 UI 之外不存在。
      ...
      const fetchTavernCard = async () => {
        ...
        // 只带一个定位参数，且**会话 id 优先**：服务端的判定顺序是
        // `if (presetId) … else if (sessionId) …`，两个都传时 presetId 会盖掉会话——
        // 而 presetId 恰恰是可能「粘住」的那个。会话 id 由服务端查 session-bindings.json，
        // 切换会话必然跟着变，这正是「不管点哪个会话都显示同一张卡」的解药。
        if (lastSessionId) url += '?sessionId=' + encodeURIComponent(lastSessionId)
        else if (lastPresetId) url += '?presetId=' + encodeURIComponent(lastPresetId)
