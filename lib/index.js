...
function presetIdForSession(sessionId) {
  try {
    ...
    if (sessionId && fs.existsSync(bindPath)) {
      ...
      if (preset && preset !== 'default') return String(preset)
    }
  } catch (_) {}
  return ''
}

/** Names of the presets that actually exist — for diagnostics when one misses. */
function availablePresetIds() {
  try {
    return fs.readdirSync(PRESETS_ROOT, { withFileTypes: true })
      .filter(d => d.isDirectory())
      .map(d => d.name)
      .sort()
  } catch (_) {
...
/**
 * The preset the tavern itself uses when a session has none.
 *
 * `GET /api/tavern/presets` answers `defaultPresetId: 'tavern-lite'`, and the
 * tavern maps an explicit `default` binding onto it the same way
 * (`if (currentPresetId === 'default') currentPresetId = 'tavern-lite'`). So
 * falling back here is *not* a guess — it is the tavern's own rule, and the only
 * reason this plugin is allowed to use it is that the response now says so via
 * `presetSource`.
 * @returns {string} absolute directory, or '' when it does not exist
 */
function defaultPresetDir() {
  return resolvePresetDirById('tavern-lite')
}

/**
 * Resolve one preset id to an existing directory *inside* PRESETS_ROOT.
 *
 * Returns '' when the preset does not exist, and the caller must report that as
 * `found:false`. It used to fall back to `TAVERN_PRESET_DIR`, which meant a
 * mistyped or renamed preset id silently served a *different* preset's card
 * (`?presetId=does-not-exist` → tavern-lite's 川上富江) while the panel said
 * nothing — the user had no way to tell they were looking at someone else's card.
 *
 * Ids are directory names, never paths: separators and `..` are rejected so a
 * request cannot walk out of the preset root.
 * @param {string} id
 * @returns {string} absolute directory, or ''
 */
function resolvePresetDirById(id) {
...
function regexScriptsOf(cardJson) {
  // Spec-v2 wraps fields in `data`, spec-v1 (flat) keeps them at the top level —
  // reading only `data` dropped every regex script of a flat card, which is what
  // makes the rendering engine work at all.
...
function extractMuvFromYml(yml) {
  ...
  try {
    ...
    if (m) {
      ...
      return { name, schemas, initvarData: parsed, initvarBlocks: [blockText], zodSource: '', schemaSource: 'initvar' }
    }
    // No initvar default — serve empty-but-valid so the panel opens empty
    return { name, schemas: [], initvarData: {}, initvarBlocks: [], zodSource: '', schemaSource: 'none' }
  } catch (_) {
    return null
  }
}
...
function inferSchemas(data) {
  ...
  for (const [key, value] of Object.entries(data)) {
    if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
      ...
      // `- item` list: shown as newline-joined text, kept as an array in the data.
      fields.push({
        name: key,
        type: 'string',
    ...
    },
    {
      ...
      handler: (req, res) => {
        try {
          ...
          // No `fallback` name any more: falling back to a fixed preset is what
          // made the panel show an unrelated card. An empty presetId now means
          // "cannot tell", and tavern-card says so instead of guessing.
          json(res, 200, { ok: true, presetId, fallback: '' })
        } catch (e) {
          json(res, 500, { ok: false, error: e.message })
        }
      }
    },
    {
      ...
      handler: async (req, res) => {
        try {
          ...
          // 需要「我就是要这个 preset，别管会话」的调用方显式声明。
          const preferPreset = url.searchParams.get('preferPreset') === '1'

          // Resolve the preset: **the session's bound preset > explicit id >**
          // whichever preset the user is actually in > the tavern's own default.
          //
          // ★ 顺序为什么会话在前（实测的事故）：客户端曾经两个 id 都送、且 presetId 排在前面，
          //   于是"带 presetId 就按 presetId 走"让 sessionId 形同虚设。而 presetId 恰恰是
          //   **会粘住**的那个 —— 它来自酒馆面板的 `dataset.presetId` / localStorage，切换会话时
          //   那个值可能仍是上一个会话的预设。用户实测：在「瑟瑟提瓦特」的会话里看到了
          //   「足控天堂」的状态栏界面（串台）。
          //
          //   会话 id 是唯一随会话切换必然变化的依据（服务端按 session-bindings.json 查），
          //   所以它必须优先。确实需要"按 id 强制取某个预设"的调用方（诊断脚本、卡库检查）
          //   显式加 `preferPreset=1` —— 不把它们的语义悄悄改掉。
          //
          // Every step that missed used to end in `presetDir = TAVERN_PRESET_DIR`
          // *silently*, so an unknown id (a renamed preset, a stale id cached in
          // the client, a typo) served that preset's card while the panel said
          // nothing — the user had no way to tell they were looking at someone
          // else's card. The default is still the right answer for a session the
          // tavern itself considers unbound (`default` ⇒ `tavern-lite`), but it is
          // no longer the answer for an id that was asked for by name; those
          // report `found:false` with the reason, and every hit reports how it was
          // located in `presetSource`.
          let presetDir = ''
          let presetSource = ''
          let missing = ''
          let requested = ''
          const fromSession = () => {
            const bound = presetIdForSession(sessionId)
            requested = bound
            if (!bound || bound === 'default') {
              // The session has no preset of its own — the tavern uses its
              // default for it, so that is the honest answer.
              presetDir = defaultPresetDir()
              if (presetDir) presetSource = 'session-default'
            } else {
              presetDir = resolvePresetDirById(bound)
              if (presetDir) presetSource = 'session'
              else missing = 'preset bound to session ' + sessionId + ' not found: ' + bound
            }
          }
          const fromExplicit = () => {
            requested = String(presetId)
            presetDir = resolvePresetDirById(presetId)
            if (presetDir) presetSource = 'explicit'
            else missing = 'preset not found: ' + requested
          }
          if (preferPreset && presetId) {
            fromExplicit()
          } else {
            if (sessionId) fromSession()
            if (!presetDir && !missing && presetId) fromExplicit()
          }
          // ★ 这里原来会无条件回落到 `activePresetId()`（"最近写入的会话"的绑定）。已收窄。
          //
          //   为什么不能当默认兜底（实测）：它的实现是按**会话文件的 mtime** 倒序，取第一个
          //   有绑定的预设。而"最近写入"根本不是"用户正在看的会话" —— 实测当时它返回的是
          //   标题为「安装 dsh-tavern-v2 及其附属插件」的**开发会话**的绑定，因为那类会话在
          //   持续写文件（跑 agent、被试跑的门禁）。于是**每个没带 sessionId 的请求都拿到
          //   同一张卡**，用户看到的就是"基本上每个会话都变成看 X 的界面"（串台）。
          //   它还**不稳定**：同一请求在不同时刻会解析到不同预设。
          //
          //   现在没有会话依据就落到**稳定默认**（tavern-lite），`presetSource` 记 `default`。
          //   面板据此能如实提示"这个会话还没绑卡"，而不是悄悄给用户看别人的卡。
          //   确实需要"最近活跃预设"的调用方（诊断脚本）显式加 `preferActive=1` ——
          //   不把"猜"当成静默兜底。
          if (!presetDir && !missing && url.searchParams.get('preferActive') === '1') {
            const active = activePresetId()
            if (active) {
              requested = active
              presetDir = resolvePresetDirById(active)
              if (presetDir) presetSource = 'active'
              else missing = 'active preset not found: ' + active
            }
          }
          if (!presetDir && !missing) {
            presetDir = defaultPresetDir()
            if (presetDir) presetSource = 'default'
          }
          if (!presetDir) {
            json(res, 200, {
              ok: true, found: false,
              error: missing || 'no preset could be resolved',
              reason: missing ? 'preset-not-found' : 'no-preset-resolved',
              requestedPresetId: requested || undefined,
              availablePresets: availablePresetIds(),
              regexScripts: []
            })
            return
          }
          ...
          if (raw) {
            ...
            json(res, 200, {
              ...
              presetSource,
              // ★ 诚实上报（交叉复核发现）：调用方**显式**给了 presetId，但被"会话优先"规则
              //   忽略了 —— 会话存在但未绑定 ⇒ `fromSession()` 把 `defaultPresetDir()` 写进了
              //   `presetDir`，于是上面那句 `if (!presetDir && !missing && presetId) fromExplicit()`
              //   永远进不去。实测 `?sessionId=<已存在但未绑定>&presetId=A-preset` 会返回
              //   tavern-lite，显式 presetId 被**静默吞掉** —— 与被修掉的串台是同一症状类
              //   （拿到不是你要的卡，而且不告诉你）。行为保持不变（要强制用它请带
              //   `preferPreset=1`），但至少不再沉默。
              ...(presetId && !preferPreset && presetSource === 'session-default'
                ? { note: 'presetId 被忽略：该会话未绑定预设（会话优先）。要强制用它请带 preferPreset=1', ignoredPresetId: presetId }
                : {}),
          ...
          if (cardName && yml) {
            ...
            json(res, 200, {
              ok: true, found: false, error: 'no card in preset',
              presetDir: path.basename(presetDir), presetSource, regexScripts: []
            })
          }
        } catch (e) {
          json(res, 500, { ok: false, error: e.message })
