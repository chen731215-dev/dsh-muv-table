...
**现象**：`异世界农场` 的整棵变量树**不在卡文本里**，而在世界书条目
`[initvar]变量初始化勿开` 的 `content` 里，是裸 YAML：
...
它**没有 `<initvar>` 标签**，所以按标签扫描的候选集
（`first_mes` / `description` / `scenario` / `alternate_greetings` / 世界书 / helper）
永远扫不到 → 该卡 `schemas` 恒为 `0`，面板一片空白。

**修法**（`lib/muv-parser.js`）：新增 `worldbookInitvarEntries()`（找 `comment` 以
`[initvar]` 开头的世界书条目）与 `looksLikeInitvarMapping()`（content 必须**读起来像
变量映射**：`键:` / `键: 值` 行算键、`- item` 算列表内容、空行与 `#` 注释跳过，
**只要有一行不属于这三类就否决**）。四道闸：① 只在上面所有来源都落空时启用；
② content 必须过形状判定；③ 解析出的映射非空；④ 响应里标
`schemaSource: 'worldbook-initvar'`，来源可见（面板上显示在卡名后面）。

**★ 为什么它会读一条 `enabled: false` 的条目**（这条请照原样保留，否则会被当 bug 报回来）：
要读的那条条目**本身就是被禁用的**，而且 `comment` 里就写着「变量初始化**勿开**」。
这是卡作者的**约定**：把变量树放进一条禁用条目 → 它不会被注入上下文，但工具链仍然
可以读到（「勿开」正是这个意思的字面声明）。所以判定只认 `[initvar]` 这个**显式标记**，
`enabled` 不参与；没有该标记的禁用条目**永远不读**。挡住垃圾的是「标记 + 形状判定」，
不是 `enabled`。若要求条目必须启用，这条兜底在唯一需要它的卡上就是空转。

**实测**：`异世界农场` schemas **0 → 3**（时间 / 种族好感度 / 个人好感度），
`schemaSource = worldbook-initvar`；`苍玄界`（对应条目叫 `[mvu_update]变量输出格式`，
是**文档**不是数据）、`_足控天堂2`、`食人世界`、`涩涩提瓦特`、`default_Seraphina`
**逐字节无回归**；`test-muv-parser.mjs` 56 → 76 项。
